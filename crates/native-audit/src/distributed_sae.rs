    ...
    fn add_dp_noise(&self, grad: &Tensor) -> Result<Tensor> {
        let noise_data: Vec<f32> = (0..grad.shape().elem_count())
            .map(|i| {
                ...
                let r1 = (i as u64).wrapping_mul(6364136223846793005).wrapping_add(777);
                let r2 = (i as u64).wrapping_mul(6364136223846793005).wrapping_add(888);
...
mod tests {
    ...
    fn test_federated_round() {
        ...
        assert!(loss.is_finite() && loss >= 0.0, "Loss must be finite and non-negative, got {}", loss);
