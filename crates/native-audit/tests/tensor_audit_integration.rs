...
fn test_smollm2_tensor_extraction_and_tcm() {
    ...
    let hidden_state = audit.forward_extract(prompt).expect("Fallo en extracción de tensor");

    let dims = hidden_state.dims();
    println!("📊 Tensor shape: {:?}", dims);
    assert_eq!(dims.len(), 3, "El tensor debe ser [batch, seq_len, hidden_dim]");

    let z_axis = audit.compute_tcm_z_axis(&hidden_state).expect("Fallo en cálculo TCM");
    println!("📐 TCM Z-axis: {:.4}", z_axis);

    assert!(z_axis.is_finite(), "Z-axis no es finito");
}
