#!/usr/bin/env bash
# ed2kIA v1.6.0 STABLE - Release Packaging Script
...
cp docs/migration_guide_v1.5_to_v1.6.md "${OUTPUT_DIR}/docs/" 2>/dev/null || true
cp docs/official_launch_announcement_v1.6.md "${OUTPUT_DIR}/docs/" 2>/dev/null || true
cp docs/architecture_v1.6.0.md "${OUTPUT_DIR}/docs/" 2>/dev/null || true
cp docs/TRANSPARENCY_FRAMEWORK.md "${OUTPUT_DIR}/docs/" 2>/dev/null || true
cp docs/v1.6.0_sprint3_release_notes.md "${OUTPUT_DIR}/docs/" 2>/dev/null || true
...
cat > "${OUTPUT_DIR}/MANIFEST.json" << MANIFEST
{
  ...
  "modules": [
    ...
    "Federation Scaling v7",
    "Gradient Sync v7",
    "Async ZKP v13",
    "Federation ZKP Bridge v6",
    "Dashboard v7",
    "WS Federation Stream v2",
    "Cross-Chain Bridge v3",
    "Interoperability Layer v2",
    "State Sync v2",
    "Merkle Aggregator v2",
    "Snapshot Manager v2"
  ],
  "sprints": {
    "sprint1": "Cross-Chain Bridge v3, Interoperability Layer v2, State Sync v2, Merkle Aggregator v2, Snapshot Manager v2",
    "sprint2": "Federation Scaling v7, Gradient Sync v7, Async ZKP v13, Federation ZKP Bridge v6, Dashboard v7, WS Federation Stream v2",
    "sprint3": "SAE Fine-Tuning v7, Cross-Model Federation Scaling v7, Async ZKP v14, Federation ZKP Bridge v7"
  },
  "tests": {
    "unit": 160,
    "e2e": 14,
