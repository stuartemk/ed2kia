//! Ethical Attention Masking — Sprint 28
//!
//! Applies Stuartian ethical valence (Z-axis) as a pre-softmax mask
//! to attention scores, causing the attention mechanism to naturally
//! deprioritize tokens with negative Z (perversidad sistémica).
//!
//! **Mathematical Logic (O(1) vectorized):**
//! 1. `mask = (sct_z_vectors < 0.0).to_f32() * -10.0`
//! 2. `masked_scores = attention_scores + mask` (broadcast)
//! 3. Return `masked_scores` — subsequent softmax will collapse
//!    attention probability on perverse tokens to near-zero.
//!
//! The -10.0 penalty ensures softmax(exp(-10)) ≈ 4.5e-5,
...
/// Applies Stuartian ethical mask to attention scores before softmax.
///
...
///
/// # Arguments
/// * `attention_scores` — Pre-softmax attention scores, shape `[batch, heads, seq_q, seq_k]`
///   or any compatible shape where the last dimension aligns with Z vectors.
/// * `sct_z_vectors` — Z-axis values per token, shape `[seq_k]` or `[batch, seq_k]`.
///
/// # Returns
...
/// # Example
/// ```rust,ignore
/// let masked = apply_stuartian_mask(&scores, &z_vectors)?;
/// let probs = candle_nn::ops::softmax_last_dim(&masked)?;
/// ```
pub fn apply_stuartian_mask(
    attention_scores: &Tensor,
    sct_z_vectors: &Tensor,
) -> Result<Tensor, EthicalAttentionError> {
    apply_stuartian_mask_with_penalty(attention_scores, sct_z_vectors, -10.0f32)
}

/// Applies Stuartian ethical mask with configurable penalty strength.
///
/// # Arguments
/// * `attention_scores` — Pre-softmax attention scores.
/// * `sct_z_vectors` — Z-axis values per token.
/// * `penalty` — Negative penalty for Z < 0 tokens (default -10.0). Must be negative.
///
/// # Returns
/// Masked attention scores ready for softmax.
pub fn apply_stuartian_mask_with_penalty(
    attention_scores: &Tensor,
    sct_z_vectors: &Tensor,
    penalty: f32,
) -> Result<Tensor, EthicalAttentionError> {
    ...
    // Step 1: Create binary mask where Z < 0 → 1.0, else → 0.0
...
/// - Z < 0.0: decay ≈ softmax(-10.0) / softmax(0.0) ≈ 4.5e-5
pub fn attention_decay_factor(z: f32) -> f32 {
    if z >= 0.0 {
        1.0
    } else {
        // softmax(-10) ≈ exp(-10) / (exp(-10) + exp(0)) ≈ 4.5e-5
...
///
/// # Arguments
/// * `attention_scores` — Shape `[batch, num_heads, seq_q, seq_k]`
/// * `sct_z_vectors` — Shape `[num_heads, seq_k]` or `[batch, num_heads, seq_k]`
///
/// # Returns
...
pub fn apply_multi_head_ethical_mask(
    ...
    // Create binary mask: Z < 0 → penalty, else → 0
...
mod tests {
    ...
    #[test]
    fn test_apply_stuartian_mask_negative_z() {
        let device = candle_core::Device::Cpu;
        ...
        // Token 0: Z=0.5 (positive) → no change → 1.0
        assert!((masked_vec[0] - 1.0).abs() < 1e-6);
        // Token 1: Z=-0.7 (negative) → penalty -10.0 → 2.0 + (-10.0) = -8.0
        assert!((masked_vec[1] - (-8.0)).abs() < 1e-6);
        // Token 2: Z=0.3 (positive) → no change → 3.0
        assert!((masked_vec[2] - 3.0).abs() < 1e-6);
        // Token 3: Z=0.1 (positive) → no change → 4.0
        assert!((masked_vec[3] - 4.0).abs() < 1e-6);
    }

    #[test]
    fn test_apply_stuartian_mask_all_negative() {
        let device = candle_core::Device::Cpu;
    ...
    fn test_custom_penalty() {
        ...
        let masked = apply_stuartian_mask_with_penalty(&scores, &z_vectors, -5.0).unwrap();
        let masked_vec: Vec<f32> = masked.to_vec1().unwrap();

        // Token 0: positive Z → no change → 5.0
        assert!((masked_vec[0] - 5.0).abs() < 1e-6);
        // Token 1: negative Z → penalty -5.0 → 3.0 + (-5.0) = -2.0
    ...
    fn test_invalid_penalty() {
        ...
        let result = apply_stuartian_mask_with_penalty(&scores, &z, 5.0);
        assert!(result.is_err());

        // Zero penalty should fail
        let result = apply_stuartian_mask_with_penalty(&scores, &z, 0.0);
        assert!(result.is_err());
    }

    #[test]
    fn test_attention_decay_factor() {
        // Positive Z → full attention
