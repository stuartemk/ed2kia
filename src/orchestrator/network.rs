...
use libp2p::gossipsub::{
    self, IdentTopic, Message, MessageAuthenticity,
};
...
pub async fn build_federation_swarm(
    ...
    let identify = libp2p::identify::Behaviour::new(
        libp2p::identify::Config::new("/ed2kia-federation/1.0.0".to_string(), key.public()),
    );
...
mod tests {
    ...
    fn test_fed_message_hash_deterministic() {
        ...
        let msg = FedMessage::new("origin".to_string(), MessageType::AtlasDelta, serde_json::json!({"key": "value"}));
    ...
    fn test_fed_message_different_types() {
        let msg1 = FedMessage::new("o".to_string(), MessageType::AtlasDelta, serde_json::json!(null));
        let msg2 = FedMessage::new("o".to_string(), MessageType::ReputationUpdate, serde_json::json!(null));
    ...
    fn test_federation_bridge_new() {
        ...
        assert!(bridge.event_tx.send(FedEvent::PeerConnected(PeerId::random())).is_ok());
    }

    #[test]
    fn test_federation_bridge_send_event() {
        let (bridge, mut rx) = FederationBridge::new();
        let msg = FedMessage::new("p".to_string(), MessageType::AtlasDelta, serde_json::json!({}));
    ...
    fn test_fed_message_serialize() {
        let msg = FedMessage::new("x".to_string(), MessageType::ReputationUpdate, serde_json::json!({"score": 42}));
    ...
    fn test_fed_message_roundtrip() {
        let msg = FedMessage::new("origin".to_string(), MessageType::AtlasDelta, serde_json::json!({"data": [1,2,3]}));
        let json = serde_json::to_string(&msg).unwrap();
