//! Divergence Detection — Detección de divergencia KL para alineación.
//!
//! **Stuartian Law 2 (Reconocimiento del Error):** Monitoreo continuo de
//! divergencia KL entre activaciones y el vector de alineación esperado.

use std::fmt;

/// Error en la detección de divergencia.
#[derive(Debug)]
pub enum DivergenceError {
    /// Dimensiones incompatibles entre vectores.
    DimensionMismatch(String),
    /// Umbral de divergencia inválido.
    InvalidThreshold(f64),
    /// Vector de activaciones vacío.
    EmptyActivation,
}
...
/// Resultado de una verificación de divergencia.
#[derive(Debug, Clone)]
pub struct DivergenceResult {
    /// Valor de divergencia KL calculada.
    pub kl_divergence: f64,
    /// ¿La activación está dentro del umbral?
    pub within_threshold: bool,
    /// Umbral utilizado.
    pub threshold: f64,
}

/// Verificador de divergencia KL para alineación.
///
/// **Stuartian Law 2:** Detección temprana de desviaciones
/// del comportamiento esperado, con rechazo determinista.
#[derive(Debug)]
pub struct DivergenceChecker {
    /// Umbral máximo de divergencia KL aceptable.
    pub threshold: f64,
}
...
impl DivergenceChecker {
    ...
    /// Calcula la divergencia KL entre activación y vector de referencia.
    ///
    /// **Stuartian Law 2:** Divergencia alta = posible desalineación.
    /// El filtro rechaza determinísticamente activaciones fuera de rango.
    pub fn check(
        &self,
        _activation: &[f32],
        _reference: &[f32],
