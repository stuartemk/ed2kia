...
use ed2kia::federation::trust_sync::{TrustSyncEngine, TrustSyncConfig, TrustNodeRecord};
...
fn bench_alignment_loop_single_cycle() {
    ...
    for i in 0..10 {
        loop_v2.ingest_feedback(make_feedback("layer_1", i, 0.5, 0.8)).unwrap();
    }
...
fn bench_alignment_loop_100_cycles() {
    ...
    for i in 0..100 {
        ...
        if loop_v2.run_cycle(&format!("layer_{}", i % 5), &activations).is_ok() {
            success_count += 1;
        }
    }
    let elapsed = start.elapsed();
...
fn bench_gradient_normalizer_batch_100() {
    ...
    for i in 0..10 {
        normalizer.register_node(make_node_capacity(&format!("node_{}", i))).unwrap();
    }
...
fn bench_comparison_federation_pipeline() {
    ...
    // Normalize
    for i in 0..10 {
        normalizer.normalize(make_gradient_batch(&format!("node_{}", i), 256, 1)).unwrap();
    }
    ...
    // Scaler sync
    for i in 0..10 {
        scaler.receive_update(make_gradient_update(&format!("node_{}", i), "model_a", 128, 1)).unwrap();
    }
    scaler.sync().unwrap();
...
fn bench_comparison_full_sprint4_pipeline() {
    ...
    // Federation
    for i in 0..5 {
        scaler.receive_update(make_gradient_update(&format!("node_{}", i), "main_model", 128, 1)).unwrap();
    }
