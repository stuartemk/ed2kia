...
mod bench {
    ...
    use ed2kia::pools_v3::pool_matcher_v3::{PoolMatcherV3, MatcherV3Config, PoolRequestV3, ShardCandidateV3, RequestType};

    // LP-104: DAO Ledger v4
    use ed2kia::governance_v4::dao_ledger_v4::{DaoLedgerV4, DaoLedgerV4Config, DaoEventV4};
    use ed2kia::governance_v4::hybrid_executor::{HybridExecutor, HybridConfig, ExecutionMode};
    use ed2kia::governance_v4::audit_trail::{AuditTrail, AuditTrailConfig, AuditAction, Severity};

    // LP-105: Async ZKP v7
    use ed2kia::zkp_v7::async_zkp_v7::{AsyncZKPV7, ZKPV7Config, ZKPStatement, ShardContext, BackendType, CircuitType};
    use ed2kia::zkp_v7::cross_pool_verification::{CrossPoolVerifier, CrossPoolConfig, PoolVerifier};

    // LP-106: UI Dashboard v5
    use ed2kia::ui_v5::dashboard_v5::{DashboardV5, DashboardV5Config, MetricV5};
    use ed2kia::ui_v5::stream_engine::{StreamEngine, StreamConfig, MetricCategory, StreamEvent};
    use ed2kia::ui_v5::alert_pipeline::{AlertPipeline, PipelineConfig, AlertRule, AlertOperator, AlertSeverity};
    ...
    pub fn bench_pool_matching_1000() {
        ...
        for i in 0..20 {
            matcher.register_candidate(make_candidate(&format!("shard-{}", i), 1000.0 + i as f64 * 100.0)).ok();
        }
        ...
        println!("LP-103: 1000 pool matches in {:?} ({:.3} ms/match)", elapsed, elapsed.as_secs_f64() * 1000.0 / 1000.0);
    }
    ...
    pub fn bench_pool_matching_100_candidates() {
        ...
        for i in 0..100 {
            matcher.register_candidate(make_candidate(&format!("shard-{}", i), 500.0)).ok();
        }
        ...
        let elapsed = start.elapsed();
        println!("LP-103: 100 matches with 100 candidates in {:?} ({:.3} ms/match)", elapsed, elapsed.as_secs_f64() * 1000.0 / 100.0);
    }
    ...
    pub fn bench_dao_ledger_1000_events() {
        ...
        let elapsed = start.elapsed();
        println!("LP-104: 1000 DAO events in {:?} ({:.3} ms/event)", elapsed, elapsed.as_secs_f64() * 1000.0 / 1000.0);
    }
    ...
    pub fn bench_audit_trail_500_entries() {
        ...
        let elapsed = start.elapsed();
        println!("LP-104: 500 audit entries in {:?} ({:.3} ms/entry)", elapsed, elapsed.as_secs_f64() * 1000.0 / 500.0);
    }
    ...
    pub fn bench_hybrid_executor_200_executions() {
        ...
        for i in 0..200 {
            executor.execute(
                format!("prop-{}", i),
                if i % 2 == 0 { ExecutionMode::OnChain } else { ExecutionMode::OffChain },
                format!("Execute proposal {}", i),
            ).ok();
        }
        let elapsed = start.elapsed();
        println!("LP-104: 200 hybrid executions in {:?} ({:.3} ms/exec)", elapsed, elapsed.as_secs_f64() * 1000.0 / 200.0);
    }
    ...
    pub fn bench_zkp_v7_proof_generation_100() {
        ...
        zkp.register_shard(ShardContext::new("bench-shard".to_string(), 5000.0, 0.95, BackendType::Halo2)).ok();
        ...
        let elapsed = start.elapsed();
        println!("LP-105: 100 ZKP v7 proofs in {:?} ({:.3} ms/proof)", elapsed, elapsed.as_secs_f64() * 1000.0 / proofs.len() as f64);
    }

    pub fn bench_zkp_v7_aggregation_50() {
        let mut zkp = AsyncZKPV7::new(ZKPV7Config::default());
        zkp.register_shard(ShardContext::new("agg-shard".to_string(), 3000.0, 0.9, BackendType::Halo2)).ok();

        for i in 0..50 {
            zkp.submit_statement(make_zkp_statement(&format!("agg-{}", i))).ok();
        }
        ...
        let elapsed = start.elapsed();
        println!("LP-105: Aggregate {} proofs in {:?} ({:.3} ms/proof)", proofs.len(), elapsed, elapsed.as_secs_f64() * 1000.0 / proofs.len() as f64);
    }
    ...
    pub fn bench_cross_pool_verification_50_sessions() {
        ...
        for i in 0..10 {
            cross_pool.register_pool(PoolVerifier::new(format!("pool-{}", i), 0.9, "halo2".to_string())).ok();
        }

        let start = Instant::now();
        for i in 0..50 {
            cross_pool.create_session(format!("session-{}", i), format!("proof-{}", i)).ok();
            // Submit votes from all pools
            for j in 0..10 {
                cross_pool.submit_vote(&format!("session-{}", i), &format!("pool-{}", j), true).ok();
            }
        }
        let elapsed = start.elapsed();
        println!("LP-105: 50 cross-pool sessions (10 pools each) in {:?} ({:.3} ms/session)", elapsed, elapsed.as_secs_f64() * 1000.0 / 50.0);
    }
    ...
    pub fn bench_dashboard_1000_metrics() {
        ...
        let elapsed = start.elapsed();
        println!("LP-106: 1000 dashboard metrics in {:?} ({:.3} ms/metric)", elapsed, elapsed.as_secs_f64() * 1000.0 / 1000.0);
    }
    ...
    pub fn bench_stream_engine_500_events() {
        ...
        for i in 0..500 {
            ...
            stream.publish(StreamEvent::new(category, format!("event-{}", i), i as f64)).ok();
        }
        stream.process_queue();
        let elapsed = start.elapsed();
        println!("LP-106: 500 stream events (3 subs) in {:?} ({:.3} ms/event)", elapsed, elapsed.as_secs_f64() * 1000.0 / 500.0);
    }

    pub fn bench_alert_pipeline_200_metrics() {
        let mut pipeline = AlertPipeline::new(PipelineConfig::default());

        pipeline.add_rule(AlertRule::new(
            "rule-cpu".to_string(),
        ...
        let start = Instant::now();
        for i in 0..200 {
            pipeline.process_metric(format!("metric-{}", i % 5), (i % 100) as f64).ok();
        }
        let elapsed = start.elapsed();
        println!("LP-106: 200 alert pipeline metrics in {:?} ({:.3} ms/metric)", elapsed, elapsed.as_secs_f64() * 1000.0 / 200.0);
    }
    ...
    pub fn bench_full_sprint2_pipeline() {
        ...
        // Pool matching
        let mut matcher = PoolMatcherV3::new(MatcherV3Config::default());
        matcher.register_candidate(make_candidate("shard-1", 2000.0)).ok();
        let req = make_request("bench-req");
        matcher.match_request(&req).ok();

        // ZKP v7
        let mut zkp = AsyncZKPV7::new(ZKPV7Config::default());
        zkp.register_shard(ShardContext::new("shard-1".to_string(), 2000.0, 0.95, BackendType::Halo2)).ok();
        ...
        // Cross-pool
        let mut cross_pool = CrossPoolVerifier::new(CrossPoolConfig::default());
        cross_pool.register_pool(PoolVerifier::new("v1".to_string(), 0.95, "halo2".to_string())).ok();
        cross_pool.create_session("bench-session".to_string(), proofs[0].proof_id.clone()).ok();
        cross_pool.submit_vote("bench-session", "v1", true).ok();

        // DAO Ledger
        let mut ledger = DaoLedgerV4::new(DaoLedgerV4Config::default());
        ledger.record_event(DaoEventV4::new(
            "bench-evt".to_string(),
