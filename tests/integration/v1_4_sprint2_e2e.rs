...
mod e2e {
    ...
    use ed2kia::bridge::pool_zkp_bridge::{PoolZKPBridge, PoolZKPConfig, BridgeProof};
    ...
    // LP-105: Async ZKP v7
    use ed2kia::zkp_v7::async_zkp_v7::{AsyncZKPV7, ZKPV7Config, ZKPStatement, ShardContext, BackendType};
    ...
    fn test_e2e_pool_matching_to_zkp_v7() {
        ...
        matcher.register_candidate(ShardCandidateV3::new(
            "shard-1".to_string(), 1000.0, 0.9, 15.0, 0.3, "halo2".to_string(),
        )).unwrap();
        ...
        zkp.register_shard(ShardContext::new(
            "shard-1".to_string(), 1000.0, 0.9, BackendType::Halo2,
        )).unwrap();
    ...
    fn test_e2e_dao_ledger_v4_lifecycle() {
        ...
        // Get entries by type
        let proposals = ledger.get_entries_by_type(&ed2kia::governance_v4::dao_ledger_v4::DaoEventTypeV4::ProposalCreated);
    ...
    fn test_e2e_zkp_v7_with_cross_pool() {
        ...
        zkp.register_shard(ShardContext::new("shard-a".to_string(), 800.0, 0.85, BackendType::Halo2)).unwrap();
        zkp.register_shard(ShardContext::new("shard-b".to_string(), 600.0, 0.95, BackendType::Groth16)).unwrap();

        // Register pools in cross-pool verifier
        cross_pool.register_pool(PoolVerifier::new("pool-a".to_string(), 0.9, "halo2".to_string())).unwrap();
        cross_pool.register_pool(PoolVerifier::new("pool-b".to_string(), 0.85, "groth16".to_string())).unwrap();
        ...
        cross_pool.create_session("session-1".to_string(), proof_id.clone()).unwrap();
    ...
    fn test_e2e_stream_with_alerts() {
        ...
        // Publish events
        stream.publish(StreamEvent::new(
            MetricCategory::Zkp,
        ...
        // Process alerts
        pipeline.process_metric("zkp_throughput".to_string(), 80.0).unwrap();
    ...
    fn test_e2e_full_sprint2_pipeline() {
        ...
        let mut matcher = PoolMatcherV3::new(MatcherV3Config::default());
        matcher.register_candidate(ShardCandidateV3::new(
            "compute-shard".to_string(), 2000.0, 0.95, 10.0, 0.2, "halo2".to_string(),
        )).unwrap();
        ...
        let mut zkp = AsyncZKPV7::new(ZKPV7Config::default());
        zkp.register_shard(ShardContext::new(
            match_result.candidate_id.clone(), 2000.0, 0.95, BackendType::Halo2,
        )).unwrap();
        ...
        let mut cross_pool = CrossPoolVerifier::new(CrossPoolConfig::default());
        cross_pool.register_pool(PoolVerifier::new("verifier-1".to_string(), 0.95, "halo2".to_string())).unwrap();
        cross_pool.register_pool(PoolVerifier::new("verifier-2".to_string(), 0.90, "groth16".to_string())).unwrap();

        let proof_id = proofs[0].proof_id.clone();
        cross_pool.create_session("full-session".to_string(), proof_id.clone()).unwrap();
        cross_pool.submit_vote("full-session", "verifier-1", true).unwrap();
        cross_pool.submit_vote("full-session", "verifier-2", true).unwrap();
        ...
        let events = stream.get_events("full_pipeline_proof".to_string()).unwrap();
    ...
    fn test_e2e_alert_escalation_pipeline() {
        ...
        // Trigger warning
        pipeline.process_metric("cpu_usage".to_string(), 85.0).unwrap();
        let warnings = pipeline.get_by_severity(&AlertSeverity::Warning);
        assert!(!warnings.is_empty());

        // Trigger critical
        pipeline.process_metric("cpu_usage".to_string(), 97.0).unwrap();
    ...
    fn test_e2e_zkp_v7_aggregation_depth() {
        ...
        zkp.register_shard(ShardContext::new("agg-shard".to_string(), 1000.0, 0.9, BackendType::Halo2)).unwrap();
    ...
    fn test_e2e_cross_pool_reputation_update() {
        ...
        cross_pool.register_pool(PoolVerifier::new("rep-pool-1".to_string(), 0.8, "halo2".to_string())).unwrap();
        cross_pool.register_pool(PoolVerifier::new("rep-pool-2".to_string(), 0.9, "groth16".to_string())).unwrap();

        // Create session and vote
        cross_pool.create_session("rep-session".to_string(), "rep-proof".to_string()).unwrap();
        cross_pool.submit_vote("rep-session", "rep-pool-1", true).unwrap();
        cross_pool.submit_vote("rep-session", "rep-pool-2", true).unwrap();
